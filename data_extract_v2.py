...
def xz_files_in_dir(directory):
	return [filename for filename in os.listdir(directory) if filename.endswith('.xz') and os.path.isfile(os.psth.join(directory, filename))]
...
def process_files_in_parallel(files, folder_path, output_file):
	...
	with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
		...
		for characters in tdqm(executor.map(process_file, args), total=len(files)):
...
# sampling a hundreth of the files for each split
sample_rate = 0.01
