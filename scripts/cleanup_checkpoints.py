...
parser.add_argument(
    '-dry_run', action='store_true', help='Show what would be deleted without actually deleting'
)
parser.add_argument(
    '-metrics', type=str, default=None, help='Metrics CSV file for best_val strategy'
)
...
if args.strategy == 'recent':
    ...
    if not args.metrics:
        ...
        print(
            "Usage: python scripts/cleanup_checkpoints.py -strategy best_val -metrics logs/training_metrics_TIMESTAMP.csv"
        )
        sys.exit(1)
    ...
    for c in checkpoint_info:
        if c['iteration'] in val_losses:
            checkpoint_with_loss.append({**c, 'val_loss': val_losses[c['iteration']]})
...
keep_list = sorted(
    [c for c in checkpoint_info if c['path'] in keep_checkpoints], key=lambda x: x['iteration']
)
for c in keep_list:
...
if final_files:
    for f in final_files:
        print(
            f"  ✓ {os.path.basename(f):30s} (final model, {os.path.getsize(f) / (1024 * 1024):6.1f} MB)"
        )
